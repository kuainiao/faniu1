        ...
        <el-tabs type="border-card">
          <el-tab-pane label="该条咨询回复列表">
            <el-table :data="replylist" border style="width: 100%;">
              ...
              <el-table-column prop="replycontent" label="内容">
              </el-table-column>
              <el-table-column prop="answertime" label="回复咨询时间" width="180">
                <template slot-scope="scope">
                  <div>{{scope.row.answertime | timeFilter('day')}}</div>
                </template>
              </el-table-column>
            </el-table>
          </el-tab-pane>
  ...
  export default {
    ...
    methods: {
      replySelf () {
        ...
        api.replySelf({id: that.id, askid: askid}).then((res) => {
          if (res.errcode === 0) {
