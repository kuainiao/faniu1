 ...
 */
var getWxLoginResult = function getLoginCode(callback,silent) {  
    wx.login({
...
var login = function login(options) {
    ...
    var doLogin = () => getWxLoginResult(function (wxLoginError, wxLoginResult) {
        ...
        if(iv) options.data.iv = iv;
        if (encryptedData) options.data.data = encryptedData;
    ...
    if (session) {
        wx.checkSession({
            success: function () {
              if (options.flush){
                  options.header = (session && session.session_id) ? { 'Cookie': 'PHPSESSID=' + session.session_id} : {};
                  doLogin();
                }else{
